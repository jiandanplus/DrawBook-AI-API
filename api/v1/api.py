...
from .training_service import prepare_training_data_service, get_training_status_service, stop_training_service
...
@router_v1.post("/training_data", operation_id="training_data", response_model=TrainingDataResponse)
async def training_data(
    ...
    try:
        return prepare_training_data_service(trigger, get_face_request_id, model_type.value)
...
def get_training_status(task_id: str):
    ...
    """
    try:
        return get_training_status_service(task_id)
    except RuntimeError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        logger.error(f"获取训练状态时出错: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
# 停止训练接口
@router_v1.post("/stop_training/{task_id}", operation_id="stop_training")
def stop_training(task_id: str):
    try:
        return stop_training_service(task_id)
    except RuntimeError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
