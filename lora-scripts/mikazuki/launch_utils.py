    ...
    with open(requirements_file, 'r', encoding='utf8') as f:
        ...
        for line in lines:
            ...
            if not is_installed(line):
                if index_url != "":
                    run_pip(f"install {line} --index-url {index_url} --extra-index-url https://pypi.org/simple/", line, live=True)
                else:
                    run_pip(f"install {line} --extra-index-url https://pypi.org/simple/", line, live=True)
...
def setup_windows_bitsandbytes():
    ...
    if not installed_bnb or not bnb_cuda_setup:
        ...
        run_pip(f"install {bnb_package} --extra-index-url https://pypi.org/simple/", bnb_package, live=True)
...
def pip_install(package: str, version: Optional[str] = None, index_url: Optional[str] = None, live: bool = True):
    ...
    if index_url:
        ...
        # Use Aliyun mirror as default
        command = f"{command} -i https://mirrors.aliyun.com/pypi/simple/"
...
def prepare_environment(disable_auto_mirror: bool = True, prepare_onnxruntime: bool = True):
    ...
    if not disable_auto_mirror and not network_gfw_test():
        ...
        os.environ.setdefault("PIP_FIND_LINKS", "https://mirrors.aliyun.com/pytorch-wheels/torch_stable.html")
